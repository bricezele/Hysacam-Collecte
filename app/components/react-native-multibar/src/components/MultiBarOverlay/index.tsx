...
export const MultiBarOverlay: React.FC<Props> = ({params}) => {
    ...
    const animations = React.useMemo(
        () => data.map(() => new Animated.Value(extrasVisible ? 1 : 0)),
        [data],
    );
    const overlayHeight = React.useMemo(
        () => surfaceSizeHalf * (surfaceSize / overlayRadius / 2),
    ...
    React.useEffect(() => {
        ...
        const animationsList = animations.map((anim, idx) =>
            animate(anim, {
    ...
    const itemsList = React.useMemo(
        () =>
            data.map((extrasRender, idx) => {
                ...
                const x =
                    overlayRadius *
                        Math.cos((angle * Math.PI) / COMMON_DEGREES) +
                    (surfaceSizeHalf - iconSizeHalf);
                const y =
                    overlayRadius *
                        Math.sin((angle * Math.PI) / COMMON_DEGREES) +
                    surfaceSizeHalf;
